		...
		<form action="" method="post" enctype="multipart/form-data">
			...
			<table class="form-table">
				...
				<tr>
					...
					<td><input id="recurring" name="recurring" type="checkbox" value="yes" 
						<?php
						if ( pmpro_isLevelRecurring( $level ) ) {
							echo "checked='checked'"; }
						?>
							 /> <label for="recurring"><?php _e( 'Check if this level has a recurring subscription payment.', 'paid-memberships-pro' ); ?></label></td>
						</tr>
								...
								<br /><small>
									...
									<?php if ( $gateway == 'stripe' ) { ?>
										...
										if ( ! empty( $pmpro_stripe_error ) ) {
											?>
											class="pmpro_red"<?php } ?>><?php _e( 'Stripe integration currently only supports billing periods of "Week", "Month" or "Year".', 'paid-memberships-pro' ); ?>
										<?php } elseif ( $gateway == 'braintree' ) { ?>
											<br /><strong 
											<?php
											if ( ! empty( $pmpro_braintree_error ) ) {
												?>
												class="pmpro_red"<?php } ?>><?php _e( 'Braintree integration currently only supports billing periods of "Month" or "Year".', 'paid-memberships-pro' ); ?>
											<?php } ?>
										</small>
										...
										<?php } elseif ( $gateway == 'braintree' ) { ?>
											...
											if ( ! PMProGateway_braintree::checkLevelForPlan( $level->id ) ) {
												?>
												pmpro_error<?php } ?>"><strong><?php _e( 'Note', 'paid-memberships-pro' ); ?>:</strong> <?php _e( 'You will need to create a "Plan" in your Braintree dashboard with the same settings and the "Plan ID" set to', 'paid-memberships-pro' ); ?> <em>pmpro_<?php echo $level->id; ?></em>.</p>
											<?php } ?>
										</td>
									</tr>
										...
										<td>
											...
											<br /><small>
												...
												<?php if ( $gateway == 'stripe' ) { ?>
													...
													if ( ! empty( $pmpro_stripe_error ) ) {
														?>
														class="pmpro_red"<?php } ?>><?php _e( 'Stripe integration currently does not support billing limits. You can still set an expiration date below.', 'paid-memberships-pro' ); ?></strong>
													<?php } ?>
												</small>
											</td>
										</tr>
										...
										?>
											...
											<td>
												...
												if ( pmpro_isLevelTrial( $level ) ) {
													echo "checked='checked'"; }
												?>
													 /> <label for="custom_trial"><?php _e( 'Check to add a custom trial period.', 'paid-memberships-pro' ); ?></label>
													...
													<?php if ( $gateway == 'twocheckout' ) { ?>
														<br /><small><strong 
															...
															if ( ! empty( $pmpro_twocheckout_error ) ) {
																?>
																class="pmpro_red"<?php } ?>><?php _e( '2Checkout integration does not support custom trials. You can do one period trials by setting an initial payment different from the billing amount.', 'paid-memberships-pro' ); ?></strong></small>
															<?php } ?>
														</td>
													</tr>

													<tr class="trial_info recurring_info" 
													<?php
													if ( ! pmpro_isLevelTrial( $level ) ) {
														echo "style='display:none;'";}
													?>
														...
														<td>
															...
															<?php if ( $gateway == 'stripe' ) { ?>
																<br /><small>
																	...
																	if ( ! empty( $pmpro_stripe_error ) ) {
																		?>
																		class="pmpro_red"<?php } ?>><?php _e( 'Stripe integration currently does not support trial amounts greater than $0.', 'paid-memberships-pro' ); ?></strong>
																	</small>
																<?php } elseif ( $gateway == 'braintree' ) { ?>
																	<br /><small>
																		<strong 
																		...
																		if ( ! empty( $pmpro_braintree_error ) ) {
																			?>
																			class="pmpro_red"<?php } ?>><?php _e( 'Braintree integration currently does not support trial amounts greater than $0.', 'paid-memberships-pro' ); ?></strong>
																		</small>
																	<?php } elseif ( $gateway == 'payflowpro' ) { ?>
																		<br /><small>
																			...
																			if ( ! empty( $pmpro_payflow_error ) ) {
																				?>
							class="pmpro_red"<?php } ?>><?php _e( 'Payflow integration currently does not support trial amounts greater than $0.', 'paid-memberships-pro' ); ?></strong>
						</small>
					<?php } ?>
				</td>
			</tr>
	...
	<table class="form-table">
		<tbody>
			<tr>
				...
				<td><input id="disable_signups" name="disable_signups" type="checkbox" value="yes" 
					<?php
					if ( $level->id && ! $level->allow_signups ) {
						?>
						checked="checked"<?php } ?> /> <label for="disable_signups"><?php _e( 'Check to hide this level from the membership levels page and disable registration.', 'paid-memberships-pro' ); ?></label></td>
					</tr>
					...
					<tr>
						...
						<td><input id="expiration" name="expiration" type="checkbox" value="yes" 
							...
							if ( pmpro_isLevelExpiring( $level ) ) {
								echo "checked='checked'"; }
							?>
							 /> <label for="expiration" id="expiration-label"><?php _e( 'Check this to set when membership access expires.', 'paid-memberships-pro' ); ?></label></a></td>
							</tr>
...
<h2 class="alignleft"><?php _e( 'Membership Levels', 'paid-memberships-pro' ); ?> 
<a href="
	<?php
	echo add_query_arg(
		array(
	...
	?>
" class="add-new-h2"><?php _e( 'Add New Level', 'paid-memberships-pro' ); ?></a></h2>
	...
	foreach ( $reordered_levels as $level ) {
		...
		if ( $count++ % 2 == 1 ) {
			?>
				...
				?>
							"><?php esc_attr_e( $level->name ); ?></a></td>
							<td>
							...
							<td>
								...
								if ( $level->allow_signups ) {
									?>
									<a href="<?php echo add_query_arg( 'level', $level->id, pmpro_url( 'checkout' ) ); ?>"><?php _e( 'Yes', 'paid-memberships-pro' ); ?></a>
									<?php
								} else {
									?>
									<?php _e( 'No', 'paid-memberships-pro' ); ?><?php } ?></td>

									<td><a title="<?php _e( 'edit', 'paid-memberships-pro' ); ?>" href="
							<?php
							echo add_query_arg(
								array(
									'page' => 'pmpro-membershiplevels',
							...
							?>
										" class="button-primary"><?php _e( 'edit', 'paid-memberships-pro' ); ?></a>&nbsp;<a title="<?php _e( 'copy', 'paid-memberships-pro' ); ?>" href="
											<?php
											echo add_query_arg(
												array(
													'page' => 'pmpro-membershiplevels',
											...
											?>
											" class="button-secondary"><?php _e( 'copy', 'paid-memberships-pro' ); ?></a>&nbsp;<a title="<?php _e( 'delete', 'paid-memberships-pro' ); ?>" href="javascript:askfirst('<?php echo str_replace( "'", "\'", sprintf( __( 'Are you sure you want to delete membership level %s? All subscriptions will be cancelled.', 'paid-memberships-pro' ), $level->name ) ); ?>', '
												<?php
												echo wp_nonce_url(
													add_query_arg(
														array(
												...
												?>
												'); void(0);" class="button-secondary"><?php _e( 'delete', 'paid-memberships-pro' ); ?></a></td>
											</tr>
												<?php
	}
	?>
